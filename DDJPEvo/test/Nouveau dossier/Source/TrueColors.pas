...
type
 TTrueColor = record
  ...
  class operator Multiply(const c: TTrueColor; const k: Real): TTrueColor;
  class operator Divide(const c: TTrueColor; const k: Real): TTrueColor;
...
function AlphaBlendTC(const c1, c2: TTrueColor; Alpha: Real): TTrueColor;
...
begin
 Result:= Round(c.r * 255.0) or (Round(c.g * 255.0) shl 8) or
  (Round(c.b * 255.0) shl 16) or (Round(c.a * 255.0) shl 24);
end;

//---------------------------------------------------------------------------
class operator TTrueColor.Implicit(c: Cardinal): TTrueColor;
begin
 Result.r:= (c and $FF) / 255.0;
 Result.g:= ((c shr 8) and $FF) / 255.0;
 Result.b:= ((c shr 16) and $FF) / 255.0;
