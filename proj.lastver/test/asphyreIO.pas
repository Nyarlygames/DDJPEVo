...
const
  ...
  t_shadvec = array [0 .. MAX_FLECHES] of t_shadow;
  ...
  TMainFrm = class(TForm)
    ...
    procedure Edit2Change(Sender: TObject; flec : t_fleche);
    procedure Edit3Change(Sender: TObject; flec : t_fleche);
...
procedure TMainFrm.Edit2Change(Sender: TObject; flec : t_fleche);
begin
    if Abs(round((lumierey + 30 - flec.ord))) <= 80  then
...
procedure TMainFrm.Edit3Change(Sender: TObject; flec : t_fleche);
var
mult : integer;

begin
mult := 1;
 if ((StrToINT(Edit2.Text) < 50) and (StrToInt(Edit2.Text) > 20)) then
   mult := 2
  else
  if StrToInt(Edit2.Text) >= 50  then  
   mult := StrToInt(Edit2.Text) div 50 + 2;
 if Abs(round((lumierey + 30 - flec.ord))) <= 30  then
    Edit3.Text := IntToStr(StrToInt(Edit3.Text) + (10*mult))
 else
   if Abs(round((lumierey + 30 - flec.ord))) <= 50  then
    Edit3.Text := IntToStr(StrToInt(Edit3.Text) + (7*mult))
   else
    if Abs(round((lumierey + 30 - flec.ord))) <= 80  then
     Edit3.Text := IntToStr(StrToInt(Edit3.Text) + (4*mult))
    else
      if Abs(round((lumierey +30 - flec.ord))) > 80  then
  ...
  Readln(F,Line);
        if Line <> '' then
         begin if Line = 'Gauche' then
               ...
               Readln(F,Line);
               shadvec[i].valtime := StrToInt(Line);
               i:=i+1;
         end;
end;
...
{if AsphyreInput.Keyboard.Update
      then begin with AsphyreInput.Keyboard do
                           begin if (Key[DIK_LEFT] = keyval) then
                                 begin recountg:= temp;
                                       lumieregX:= 50;
                                       capteurgX := MAX_WIDTH+300;
                                       dirkey := 'Gauche'
                                 end;
                                 if (Key[DIK_DOWN] = keyval) then
                                 begin recountb:= temp;
                                       lumierebX:= 650;
                                       capteurbX := MAX_WIDTH+300;
                                       dirkey := 'Bas'
                                 end;
                                 if (Key[DIK_UP] = keyval) then
                                 begin recounth:= temp;
                                       lumierehX:= 350;
                                       capteurhX := MAX_WIDTH+300;
                                       dirkey := 'Haut'
                                 end;
                                 if (Key[DIK_RIGHT] = keyval) then
                                 begin recountd:= temp;
                                       lumieredX:= 950;
...
//shadvec[0].dir := 203;
//shadvec[0].valtime := 126;

for w := 0 to nbr do
for v := 0 to nbshad do
...
if abs(round(vecteur[w].ord - (capteury + 30))) < 100 then
 if (((shadvec[v].dir = 203) and ( shadvec[v].valtime = temp)) and (vecteur[w].dir =203))  then
  begin
    recountg:= temp;
    lumieregX:= 50;
    capteurgX := MAX_WIDTH+300;
    ...
    Edit2Change(Edit2, vecteur[w]);
    Edit3Change(Edit3, vecteur[w]);
    vecteur[w].ord := -400;
  end
 else
  if(((shadvec[v].dir = 205) and ( shadvec[v].valtime = temp)) and (vecteur[w].dir =205)) then
   begin
     recountd:= temp;
     lumieredX:= 950;
     capteurdX := MAX_WIDTH+300;
   ...
   if (((shadvec[v].dir = 200) and ( shadvec[v].valtime = temp)) and (vecteur[w].dir =200)) then
    begin
      recounth:= temp;
      lumierehX:= 350;
      capteurhX := MAX_WIDTH+300;
    ...
    if (((shadvec[v].dir = 208) and ( shadvec[v].valtime = temp)) and (vecteur[w].dir =208)) then
     begin
      recountb:= temp;
      lumierebX:= 650;
      capteurbX := MAX_WIDTH+300;
