...
begin
  ...
  begin
    ...
    if Key[DIK_BACKSPACE] then
      begin buttonSelected := MessageDlg('Enregistrer le shadow ?',mtConfirmation, mbOKCancel, 0);
            if buttonSelected = mrOK then
            begin
             AssignFile(F,'shadow.jps');
             Rewrite(F);
             Append(F);
             for v := 0 to nbshad -1 do
             begin
               Writeln(F,shadvec[v].dir);
               Writeln(F,shadvec[v].valtime);
               Writeln(F,'');
             end;
            CloseFile(F);
            end;
...
if abs(round(vecteur[w].ord - (capteury + 30))) < 100 then
 if ((dirkey = 'Gauche') and ( vecteur[w].dir = 203))  then
  begin
    ...
    shadvec[nbshad].valtime := temp;
    shadvec[nbshad].dir := dirkey;
    nbshad := nbshad+1;
    vecteur[w].ord := -400;    
  end
 else
