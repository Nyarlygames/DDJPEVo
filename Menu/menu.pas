...
begin
ShellExecute(0, Nil, 'Sprite.exe', nil, '', SW_NORMAL);
end;

procedure TMainFrm.Button2Click(Sender: TObject);
begin
ShellExecute(0, Nil, 'Client.exe', nil, '', SW_NORMAL);
end;

procedure TMainFrm.Button3Click(Sender: TObject);
begin
ShellExecute(0, Nil, 'Gameplay.exe', nil, '', SW_NORMAL);
end;

procedure TMainFrm.Button4Click(Sender: TObject);
begin
ShellExecute(0, Nil, 'AI.exe', nil, '', SW_NORMAL);
end;

procedure TMainFrm.Button5Click(Sender: TObject);
begin
ShellExecute(0,'open',PChar('http://ddjpevo.free.fr/'),nil,nil, SW_NORMAL);
end;

procedure TMainFrm.Button6Click(Sender: TObject);
begin
ShellExecute(0, Nil, 'Editeur.exe', nil, '', SW_NORMAL);
end;

procedure TMainFrm.Button7Click(Sender: TObject);
begin
ShellExecute(0, Nil, 'Serveur.exe', nil, '', SW_NORMAL);
end;
...
begin
  ImageGroups.ParseLink('/Ressources2.xml'); //Chargement des images
  FontGroups.ParseLink('/Ressources2.xml'); //Chargement des polices
...
begin
  ...
  begin
    ...
    //Titre - Header
    Sender.Fonts.Font['police'].TextOut('DANCE DANCE JP EVOLUTION Soutenance III',
    125, 10, $FFFF0000);

    //Titre - Menu
    Sender.Fonts.Font['police'].TextOut('MENU',
    175, 500, $FFFF0000);
    Sender.Fonts.Font['police'].TextOut('Reseau',
    688, 560, $FFFF0000);
    // ****Dessin de la ligne de soulignement du titre
    LineWidth := 2;
    //Dessin de la ligne avec LineEx pour avoir une épaisseur
    LineEx(25, 540, 850, 540, $FFFE2415);
