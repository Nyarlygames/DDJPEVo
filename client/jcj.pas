...
uses
  ...
  AsphyreKeyboard, AsphyreInputs, ExtCtrls, StdCtrls, droite,gauche,haut,bas,test, ExtDlgs,
...
type
  ...
  public
    { Déclarations publiques }
    procedure FormCreate(Sender: TObject);
    procedure ConfigureDevice(Sender: TAsphyreDevice; Tag: TObject; var Config: TScreenConfig);  overload;
    procedure TimerEvent(Sender: TObject);  overload;
    procedure ProcessEvent(Sender: TObject);  overload;
    procedure RenderCallback(Sender: TAsphyreDevice; Tag: TObject);  overload;
  end;
...
begin
 if Abs(round((lumierey + 30 - flec.ord))) <= 30  then
  Form2.Edit1.Text := 'Marvelous'
 else
   if Abs(round((lumierey + 30 - flec.ord))) <= 50  then
    Form2.Edit1.Text := 'Perfect'
   else
    if Abs(round((lumierey + 30 - flec.ord))) <= 80  then
     Form2.Edit1.Text := 'Great'
    else
      if Abs(round((lumierey +30 - flec.ord))) > 80  then
       Form2.Edit1.Text := 'Bad'
...
if temp >30 then
Form2.Memo1.Lines.add(IntToStr((temp div 10)-3));
...
if temp <=10 then
  begin Form2.Memo1.Lines.add('1')
  end
else if (temp >10) and (temp <=20) then
  begin Form2.Memo1.Lines.add('2');
  end
else if (temp >20) and (temp <=30) then
  begin Form2.Memo1.Lines.add('3');
  end;
