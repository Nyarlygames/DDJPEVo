...
Time.Text := (inttostr( ( FSOUND_STREAM_GetTime (music ) )div 60000 ) +
     ...
     ':' + (inttostr(((FSOUND_Stream_GetLengthMs(music)) mod 60000)div 1000)) +
     ':' + inttostr((( FSOUND_Stream_GetLengthMs(music))div 100)mod 10 ));
     ShowMessage(inttostr(FSOUND_Stream_GetLengthMs(Music)));
Readln(Open,Line);
...
if (SaveDialog1.Filename = 'SansTitre.jp') then
 begin buttonSelected := MessageDlg('Enregistrer dans SansTitre.jp ?',mtConfirmation, mbOKCancel, 0);
       if buttonSelected = mrOK then
        begin AssignFile(F,SaveDialog1.FileName);
              Rewrite(F);
              Append(F);
              Writeln(F,Opendialog.Filename);
              Writeln(F,'');
              for i := 0 to nombr do
...
buttonSelected : integer;
  ...
  begin if (FileExists(SaveDialog1.FileName)) then
